    ...
    public Response run() throws DataBaseException {
        ...
        if (user != null && user.getLogin() != null && user.getPassword() != null) {
            if (userManager.isPresent(user.getLogin())) {
                ...
                throw new DataBaseException("ѕользователь " + user.getLogin() + " уже существует.");
            }
            userManager.add(user);
            return new AnswerMsg().info("ѕользователь " + user.getLogin() + " успешно зарегистрирован.").setStatus(Response.Status.AUTH_SUCCESS);
        }
